...
yAmpFinal = zeros(size(y)); % Initialises the array of amplitudes of each frequency for the processed audio
for c = 1:channelNum % For the number of channels of audio, the following code will execute
    ...
    yAmpFinal(:,c) = abs(fft(yFinal(:, c))); % Finds the real numbers in the discrete fourier transform of the processed data and stores them in an array
...
yAmpFinal = yAmpFinal(1:floor(length(yAmpFinal)/2))*2; % Halves the length of the array to remove the symmetrical points and doubles the amplitudes to redistribute the energy from the removed points
