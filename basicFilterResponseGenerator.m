...
switch filterType % Depending on the number the user enters, the code corrosponding to this number will run
    ...
    case 3 % If 3 is selected, the following code will execute
        % Band pass filter
        [fc1, fc2] = findCutoff(fc, fs); % Finds the lower and upper cutoff frequencies around the central selected cutoff frequency
...
function [fc1, fc2] = findCutoff(fc, fs) % Declares function to request the bandwidth of the BP/BS filter from the user and calculates the cutoff frequencies required
