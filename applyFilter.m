...
for i = 1:length(y)-ws+1 % For one more than the number of elements in y less the window size, the following code will execute
    yFinal(i:i+ws-1,:) = yFinal(i:i+ws-1,:) + h.*y(i+ws-1,:); % Slides the window over one channel of the imported audio and multiplies the values in the coefficient vector and amplitude vector element wise, summing the results
